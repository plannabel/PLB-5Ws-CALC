...
exports.checkWeddingDateRelativeToToday = checkWeddingDateRelativeToToday;

function dateDifference(d1, d2) {
  //@source http://stackoverflow.com/questions/26311489/obtain-difference-between-two-dates-in-years-months-days-in-javascript
...
function dateDiffNiceString(dateDiffn) {
  ...
  output.length == 0
    ...
    : output.length == 1
      ? (outString = output[0])
      : output.length == 2
        ? (outString = output[1] + " and " + output[0])
        : output.length == 3
          ? (outString = output[2] + ", " + output[1] + " and " + output[0])
          : false;
...
/**
 * Works out whether the proposed wedding date is in the past, future, or is today.
 * @function
 * @name checkWeddingDateRelativeToToday
 * @param {string} weddingDate - wedding date string to parse.
 * @returns Number - Integer where 0 = no difference, 1 = wedding date is tomorrow, -1 = wedding date is yesterday.
 */
function checkWeddingDateRelativeToToday(weddingDate) {
  return Number(moment(weddingDate).diff(moment().startOf("day"), "days")) - 1;
}
